...
// Execute frontend start web task. Update to change the  frontend start web task.
def executeUiStartWeb(implicit dir: File): Int = ifNodeModulesInstalled(runOnCommandline(FrontendCommands.start))
...
//Eject Build
def executeEjectBuild(implicit dir: File): Int = ifNodeModulesInstalled(runOnCommandline(FrontendCommands.eject))
...
lazy val `ui-start-web` = taskKey[Unit]("Run UI tests when testing application.")

`ui-start-web` := {
  implicit val userInterfaceRoot = baseDirectory.value / "ui"
  if (executeUiStartWeb != Success) throw new Exception("UI web start failed!")
}
...
lazy val `ui-eject` = taskKey[Unit]("Run UI build when packaging the application.")

`ui-eject` := {
  implicit val userInterfaceRoot = baseDirectory.value / "ui"
  if (executeEjectBuild != Success) throw new Exception("Oops! UI Eject Failed.")
}
